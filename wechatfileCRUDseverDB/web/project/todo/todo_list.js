	...
	var initRecordList=function(){
		$('.datatable').dataTable( {
			...
			"aoColumns": [{"mRender": function(data, type, full) {
					...
					var url=full.attachment_url;
					if(url.substring(0,1)=="/") url="../../"+url.substring(1,url.length);		//去掉最前头的“/”
					sReturn = '<div><a href="'+url+'" target="_blank">查看图片</a></div>';
					return sReturn;
				},"orderable": false}],
			"aLengthMenu": [[5,10,15,20,25,40,50,-1],[5,10,15,20,25,40,50,"所有记录"]],
			"fnDrawCallback": function(){$(".checkboxes").uniform();$(".group-checkable").uniform();},
			//"sAjaxSource": "get_record.jsp"
			"sAjaxSource": "../../project_todo_servlet_action?action=get_todo_record&want_attachment=yes"
		});
